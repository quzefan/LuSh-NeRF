...
from utils.run_lushnerf_helpers import *
...
class RBK(nn.Module):
    def __init__(self, num_img, view_embed_ch,
                D_rbk, W_rbk, D_rbk_r, W_rbk_r, D_rbk_v, W_rbk_v, D_rbk_w, W_rbk_w,
                output_ch_rbk_r, output_ch_rbk_v, skips_rbk, rbk_use_origin, rbk_se_rv_window, num_motion_rbk, 
                use_dpnerf=True, use_awp=False, near=0.0, far=1.0, ndc=False):
                
        super(RBK, self).__init__()
...
class NeRFAll(nn.Module):
    def __init__(self, args, blur_kernel_net=None):
        ...
        self.mlp_noise_coarse = NeRF_Noise(
            D=args.netdepth // 2, W=args.netwidth // 2,
            input_ch=self.input_ch, output_ch=3, skips=skips,
            input_ch_views=self.input_ch_views, use_viewdirs=args.use_viewdirs,
            use_awp=self.use_awp)
    ...
    def mlpforward_noise(self, inputs, viewdirs, mlp, force_naive, inference, netchunk=1024 * 64):
        """Prepares inputs and applies network 'fn'.
            """
        inputs_flat = inputs[:, 16]
        embedded = self.embed_fn(inputs_flat)
    ...
    def render_rays(self,
        ...
        pts_noise = rays_o[..., None, :] + rays_d[..., None, :] * z_vals[..., :, None]
        ...
        if self.use_awp and not force_naive and not inference:
            ...
            raw_noise = self.mlpforward_noise(pts_noise.detach(), viewdirs.detach(), self.mlp_noise_coarse, force_naive, inference)
            rgb_map_noise = raw_noise
        else:
            raw = self.mlpforward(pts, viewdirs, self.mlp_coarse, force_naive, inference)
            rgb_map_noise = self.mlpforward_noise(pts_noise.detach(), viewdirs.detach(), self.mlp_noise_coarse, force_naive, inference)
        ...
        ret_noise = {'rgb_map': rgb_map_noise}
        if retraw:
            ret['raw'] = raw
            ret_noise['raw'] = 0
        if N_importance > 0:
            ...
            ret_noise['rgb0'] = rgb_map_noise_0
        ...
        return ret, ret_noise
    
    def render_rays_nonoise(self,
                    ray_batch,
                    N_samples,
                    img_idx=None,
                    retraw=False,
    ...
    def forward(self, H, W, K, chunk=1024 * 32, rays=None, rays_info=None, poses=None, allkernel=False, kernel_pixel=None, consist_loss=False, Align_matrix=None, Align_mask=None, **kwargs):
        """
        ...
        """
        # training
        if self.training and consist_loss == False:
            ...
            if self.blur_kernel_net is not None and not force_baseline and self.blur_model_type == 'dpnerf':
                ...
                if self.blur_kernel_net.use_dpnerf:
                    rays_transform, ccw = self.mlp_rbk(rays, rays_info)

                    if allkernel:
                        kernel_pixel = kernel_pixel.repeat_interleave(5)
                        rays_transform = torch.where(kernel_pixel.unsqueeze(1).unsqueeze(1), rays_transform, rays_transform.detach()) 
                    
                    (rgb, depth, acc, extras) = self.render_train_scene(H, W, K, chunk, rays_transform, **kwargs)
                    
                    rgb_noise = self.render_train_noise(H, W, K, chunk, rays, **kwargs)

                    rgb_noise = 0.1 * torch.sigmoid(rgb_noise)

                    extras_copy = extras.copy()
                    extras['rgb0'] = extras['rgb0']
                    rgb_pure, depth_pure, acc_pure, extras_pure = self.mlp_rbk.rbk_weighted_sum(rgb, depth, acc, extras, ccw)
                    return self.tonemapping(rgb_pure + rgb_noise), self.tonemapping(extras_pure['rgb0'] + rgb_noise), {}, rgb_noise, rgb_noise, self.tonemapping(rgb_pure), self.tonemapping(extras_pure['rgb0'])
                    
                
            else:
                kwargs['img_idx'] = rays_info['images_idx'].squeeze(-1)
                (rgb, depth, acc, extras), rgb_noise = self.render_infer(H, W, K, chunk, rays, **kwargs)
                rgb_noise =  0.1 * torch.sigmoid(rgb_noise)

                return self.tonemapping(rgb), self.tonemapping(extras['rgb0']), {}, rgb_noise, rgb_noise, {}, {}
        
        elif self.training and consist_loss == True:
            save_warped_ray_img = kwargs['render_kwargs'].pop('save_warped_ray_img', False)
            rgb_align, align_certainty =  self.Render_Aligned_Pixel(H, W, K, chunk, poses, images_idx=rays_info, align_matrix=Align_matrix, align_mask=Align_mask, **kwargs)

            return rgb_align, align_certainty 

        #  evaluation
        else:
            save_warped_ray_img = kwargs['render_kwargs'].pop('save_warped_ray_img', False)
            assert poses is not None, "Please specify poses when in the eval model"
            rgbs, rgbs_noise, depths = self.render_path(H, W, K, chunk, poses, **kwargs)
            rgbs_noise = torch.reshape(rgbs_noise, [-1,H,W,3])
            rgbs_noise =  0.1 * torch.sigmoid(rgbs_noise)
            return self.tonemapping(rgbs), self.tonemapping(rgbs_noise), depths
        
    def render_infer(self, H, W, K, chunk, rays=None, c2w=None, ndc=True,
               near=0., far=1.,
               use_viewdirs=False, c2w_staticcam=None, 
               use_awp=False, allkernel=0, kernelpixel=None, render_noise=True,
        ...
        for i in range(0, rays.shape[0], chunk):
            ret, ret_noise = self.render_rays(rays[i:i + chunk], **kwargs)
            ...
            for k in ret_noise:
                if k not in all_ret_noise:
                    all_ret_noise[k] = []
                all_ret_noise[k].append(ret_noise[k])
        ...
        ret_noise = torch.cat(all_ret_noise['rgb_map'], dim = 0)
        
        if use_awp:
            ret_dict['rays_d'] = rays_d
        
        return (ret_list + [ret_dict]), ret_noise
        
        
    def render_train_scene(self, H, W, K, chunk, rays=None, c2w=None, ndc=True,
               near=0., far=1.,
               use_viewdirs=False, c2w_staticcam=None, 
               use_awp=False, allkernel=0, kernelpixel=None, render_noise=True,
               **kwargs):  # the render function
        ...
        for i in range(0, rays.shape[0], chunk):
            ret = self.render_rays_nonoise(rays[i:i + chunk], **kwargs)
            for k in ret:
                if k not in all_ret:
                    all_ret[k] = []
                all_ret[k].append(ret[k])
    ...
    def render_train_noise(self, H, W, K, chunk, rays=None, c2w=None, ndc=True,
               near=0., far=1.,
               use_viewdirs=False, c2w_staticcam=None, 
               use_awp=False, allkernel=0, kernelpixel=None, render_noise=True,
               **kwargs):  # the render function
        ...
        for i in range(0, rays.shape[0], chunk):
            ret_noise = self.render_rays_noise(rays[i:i + chunk], **kwargs)
    ...
    def render_path(self, H, W, K, chunk, render_poses, render_kwargs, render_factor=0):
        ...
        for i, c2w in enumerate(render_poses):
            ...
            (rgb, depth, acc, extras), rgb_noise = self.render_infer(H, W, K, chunk=chunk, rays=rays, c2w=c2w[:3, :4], **render_kwargs)
            rgbs.append(rgb)
            rgbs_noise.append(rgb_noise)
            depths.append(depth)
            if i == 0:
                print(rgb.shape, rgb_noise.shape)
        rgbs = torch.stack(rgbs, 0)
        rgbs_noise = torch.stack(rgbs_noise, 0)
        depths = torch.stack(depths, 0)

        return rgbs, rgbs_noise, depths
    ...
    def render_warped_path(self, H, W, K, chunk, render_poses, images_idx, render_kwargs, render_factor=0):
        ...
        for i, c2w in enumerate(render_poses):
            ...
            rgb, depth, acc, extras = self.render_train_scene(H, W, K, chunk=chunk, rays=rays_warped, c2w=c2w[:3, :4], **render_kwargs)
    ...
    def Render_Aligned_Pixel(self, H, W, K, chunk, render_poses, images_idx, render_kwargs, render_factor=0, align_matrix=None, align_mask=None):
        """
        render warped images and depths specified by the render_poses
        """
        if render_factor != 0:
        ...
        t = time.time()
        anchor_pose = random.randint(0, len(render_poses)-1)
        anchor_align_matrix = align_matrix[anchor_pose]
        anchor_certain = align_mask[anchor_pose]

        samples = np.random.randint(0, 640*1120, size=32)  
        samples = torch.tensor(samples).long().cuda()

        anchor_pose = anchor_align_matrix[:, samples]
        anchor_certain = anchor_certain[:, samples]

        rgb_align = torch.zeros([len(render_poses), 32, 3]).cuda()
        anchor_certain_align = torch.zeros([len(render_poses), 32]).cuda()
        ...
        for i in range(len(render_poses)):
            ...
            align_pixel_int = anchor_pose[i, :, 2:].long()

            rays = rays_org.reshape(H, W, 1, 3, 2)[torch.clamp(align_pixel_int[:, 1], 0, H-1).long(), torch.clamp(align_pixel_int[:, 0], 0, W-1).long(), ...]

            rays = rays.reshape(-1, 3, 2)
            
            rgb, _, _, _ = self.render_train_scene(H, W, K, chunk=chunk, rays=rays.detach(), c2w=c2w[:3, :4], render_noise=False, **render_kwargs)
            rgb_align[i] = rgb
            anchor_certain_align[i] = anchor_certain[i]

        return rgb_align, anchor_certain_align