...
from gim.dkm import models as gimmodels
# np.random.seed(0)
DEBUG = False
...
def config_parser():
    ...
    parser.add_argument("--scaleup-gamma", type=float, default=0.8,
                        help='input images preprocessing-gamma')
    
    parser.add_argument("--scaleup-clahe", type=float, default=15,
                        help='input images preprocessing-histogram')
    
    parser.add_argument("--noisenerf_start_iter", type=int, default=200000,
                        help='start training kernel after # iteration')
    
    parser.add_argument("--allkernel_start_iter", type=int, default=0,
                        help='start training kernel after # iteration')
    parser.add_argument("--fq_threshold", type=int, default=50,
                        help='start training kernel after # iteration')
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        #                                                           path_epi=args.render_epi, gamma=args.scaleup_gamma, clahe=args.scaleup_clahe)
        images, poses, bds, render_poses, i_test = load_llff_data(args, args.datadir, args.factor,
                                                                  recenter=True, bd_factor=.75,
                                                                  spherify=args.spherify,
                                                                  path_epi=args.render_epi, gamma=args.scaleup_gamma, clahe=args.scaleup_clahe)
    ...
    imagesf = images
    # Align_Matrix = torch.zeros([len(i_train), len(i_train), 1500, 4]).cuda()
    # Align_Mask = torch.zeros([len(i_train), len(i_train), 1500], dtype=torch.bool).cuda()
    images = (images * 255).astype(np.uint8)
    frequency_masks = get_masks_for_images(images, radius=args.fq_threshold)
    ...
    Align_Matrix = torch.zeros([len(i_train), len(i_train), H*W, 4]).cuda()
    Align_Mask = torch.zeros([len(i_train), len(i_train), H*W], dtype=torch.bool).cuda()
    ...
    if args.blur_model_type == 'dpnerf':
        blur_kernel_net = RBK(num_img=len(images), view_embed_ch=args.rbk_view_embed_ch, 
                            ...
                            use_dpnerf=args.use_dpnerf,rbk_use_origin=args.rbk_use_origin, near=near, far=far, ndc=(not args.no_ndc))
    ...
    align_model = gimmodels.model_zoo.DKMv3(weights=None, h=640, w=1120)
    ...
    align_model = align_model.eval().cuda()
    optim_params = nerf.parameters()
    
    mlp_noise_coarse_params = list(nerf.module.mlp_noise_coarse.parameters())  
    special_lr_params_id = list(map(id, mlp_noise_coarse_params))
     
    base_params = filter(lambda p: id(p) not in special_lr_params_id, nerf.parameters())  
      
    special_lr = args.lrate
     
    optimizer = torch.optim.Adam([  
        {'params': base_params},  # 剩余模块的参数，使用默认学习率  
        {'params': mlp_noise_coarse_params, 'lr': special_lr},  # mlp_noise_coarse模块的参数   
    ], lr=args.lrate) 
    ...
    train_datas['fq_mask'] = frequency_masks[i_train].reshape(-1)
    ...
    imagesf = torch.tensor(imagesf).cuda()
    threshold = torch.mean(torch.mean(torch.mean(imagesf, dim = 0), dim = 0), dim = 0)
    ...
    for i in range(start, N_iters):
        ...
        threshold_pixel = torch.tensor([all(target_rgb[i] > threshold) for i in range(len(target_rgb))])
        rgb_blur, rgb0_blur, extra_loss, noise, extras_noise, rgb, rgb0 = nerf(H, W, K, chunk=args.chunk,
                                     rays=batch_rays, rays_info=iter_data,
                                     retraw=True, force_naive=i < args.kernel_start_iter, allkernel = i < args.allkernel_start_iter, kernel_pixel = fq_mask,
                                     **render_kwargs_train)
        if i >= args.noisenerf_start_iter:
            render_warped_poses = torch.tensor(poses[i_train]).cuda()
            images_idx_warped = torch.tensor(images_idx[i_train]).cuda()   
            rgb_align, align_certainty = nerf(
                hwf[0], hwf[1], K, args.chunk,
                poses=render_warped_poses,
                render_kwargs=render_kwargs_test,
                render_factor=args.render_factor,
            ...
            mask = align_certainty >= 0.8

            mean_rgb_align = compute_mean_with_confidence(rgb_align, align_certainty, 0.8)

            loss_rgb += torch.sum(torch.abs(rgb_align - mean_rgb_align.unsqueeze(0)) * mask.unsqueeze(2)) / len(mask[mask == 1])
        
        img_loss = img2mse(rgb_blur, target_rgb)
        img_loss_l1 = img2l1(rgb_blur, target_rgb)
        psnr = mse2psnr(img_loss)
        img_loss0 = img2mse(rgb0_blur, target_rgb)
        img_loss0_l1 = img2l1(rgb0_blur, target_rgb)

        if i > args.noisenerf_start_iter: 
            loss = 0.5 * img_loss + 0.5 * img_loss_l1 + 1e-2 * loss_rgb + 0.5 * img_loss0 + 0.5 * img_loss0_l1
        else:
            loss = 0.5 * img_loss + 0.5 * img_loss_l1 + 0.5 * img_loss0 + 0.5 * img_loss0_l1
        ...
        if i % args.i_testset == 0 and i > 0:
            ...
            with torch.no_grad():
                nerf.eval()
                rgbs, rgbs_noise, _ = nerf(H, W, K, args.chunk, poses=poses,
                               render_kwargs=render_kwargs_test)
                rgbs_save = rgbs
                rgbs_noise_save = rgbs_noise
                
                # saving
                for rgb_idx, rgb in enumerate(rgbs_save):
                    rgb8 = to8b(rgb.cpu().numpy())
                    rgb_noise8 = to8b(rgbs_noise_save[rgb_idx].cpu().numpy())
                    rgb_blur8 = to8b(rgb.cpu().numpy() + rgbs_noise_save[rgb_idx].cpu().numpy())
                    filename = os.path.join(testsavedir, f'{rgb_idx:03d}.png')
                    noise_filename = os.path.join(testsavedir, f'{rgb_idx:03d}_noise.png')
                    blur_filename = os.path.join(testsavedir, f'{rgb_idx:03d}_blur.png')
                    imageio.imwrite(filename, rgb8)
                    imageio.imwrite(noise_filename, rgb_noise8)
                    imageio.imwrite(blur_filename, rgb_blur8)
        ...
        if i % 20000 == 0 and i >= args.noisenerf_start_iter:
            torch.cuda.empty_cache()
            for k in range(len(i_train)):
                for v in range(len(i_train)):
                    img_path0 = os.path.join(testsavedir, f'{i_train[k]:03d}.png')
                    img_path1 = os.path.join(testsavedir, f'{i_train[v]:03d}.png')
                    image0 = cv2.imread(img_path0, cv2.IMREAD_COLOR)[:, :, ::-1]
                    image1 = cv2.imread(img_path1, cv2.IMREAD_COLOR)[:, :, ::-1]
                    image0 = torch.from_numpy(image0.transpose((2, 0, 1)) / 255.0).float().cuda().unsqueeze(0)
                    image1 = torch.from_numpy(image1.transpose((2, 0, 1)) / 255.0).float().cuda().unsqueeze(0)

                    dense_matches, dense_certainty = align_model.match(image0, image1)
                    matches, certainty = (
                        dense_matches[:, :W].reshape(-1, 4),
                        dense_certainty[:, :W].reshape(-1),
                    )
                    ...
                    Align_Matrix[k,v,:,:2] = kpts0.long()
                    Align_Matrix[k,v,:,2:] = kpts1.long()
                    Align_Mask[k,v] = certainty.cuda()
