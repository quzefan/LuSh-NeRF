...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, gamma=0.8, clahe=15):
...
def _load_data_lush(basedir, factor=None, width=None, height=None, load_imgs=True, gamma=0.8, clahe=15):
    poses_arr = np.load(os.path.join(basedir, 'poses_bounds.npy'))
    poses = poses_arr[:, :-2].reshape([-1, 3, 5]).transpose([1, 2, 0])
    bds = poses_arr[:, -2:].transpose([1, 0])
    ...
    outputdir = os.path.join(basedir, 'images' + sfx+ '_preprocess')
    
    if not os.path.exists(outputdir):
        os.mkdir(outputdir)
    
        process_images_with_clahe(imgdir, outputdir, gamma, clahe)
    
    if not os.path.exists(outputdir):
        print(outputdir, 'does not exist, returning')
        return

    imgfiles = [os.path.join(outputdir, f) for f in sorted(os.listdir(outputdir)) if
                f.endswith('JPG') or f.endswith('jpg') or f.endswith('png')]
    if poses.shape[-1] != len(imgfiles):
        print('Mismatch between imgs {} and poses {} !!!!'.format(len(imgfiles), poses.shape[-1]))
        return
...
def load_llff_data(args, basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_epi=False, gamma=1.0, clahe=15):
    poses, bds, imgs = _load_data_lush(basedir, factor=factor, gamma=gamma, clahe=clahe)  # factor=8 downsamples original imgs by 8x
