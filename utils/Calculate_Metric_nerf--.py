...
# 文件夹路径  
folder1 = 'D:\Code/nerfmm\logs/nerfmm/0123_nerfmm\lr_0.001_gpu0_seed_17_resize_1_Nsam_32_Ntr_img_-1_freq_10__240803_2225/render_val\img_out'  # 第一个文件夹路径  
folder2 = 'D:\Code\DP-NeRF\data/HL-0123'  # 第二个文件夹路径  
  
# 确保两个文件夹中的图像数量相同  
# images1 = sorted(glob.glob(os.path.join(folder1, '[0-9][0-9][0-9].png')), key=lambda x: int(os.path.splitext(os.path.basename(x))[0]))  
images1 = sorted(glob.glob(os.path.join(folder1, '[0-9][0-9][0-9][0-9].png')), key=lambda x: int(os.path.splitext(os.path.basename(x))[0].split('_')[0])) 
...
# 计算PSNR, SSIM, 和LPIPS  
for img1_path, img2_path in zip(images1, images2[::8]):  
    # 加载图像  
    ...
    win_size = min(7, min_size - 1) if min_size > 7 else 7  # 确保 win_size 是奇数且小于等于 min_size  
    # 计算PSNR  
